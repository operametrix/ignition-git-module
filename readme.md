...
- Snapshot gateway resources (tags, themes, images) into project files before committing — three buttons in the Commit panel header capture the current gateway-side state of each resource type so it shows up as reviewable file changes in the Changes list; you can then check/uncheck individual files before committing like any other change
