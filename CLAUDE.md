...
- `GitReposUsersRecord` — maps Ignition users to projects. Retains `Email`/`UserName`/`Password`/`SSHKey` columns for schema compatibility and the commissioning import path, but the Designer no longer reads or writes them: commit author email now comes from the Ignition user profile, and auth is handled exclusively via the credential records below
- `GitRemoteCredentialsRecord` — per-(project, user, remote) record. Holds two FK columns (`SshKeyId`, `HttpsCredentialId`) that point into the user-level credential tables (the path used by the new UI), plus legacy inline `UserName`/`Password`/`SSHKey` columns retained as Tier 2 fallback in `setAuthentication`. Created when adding a remote via `RemotesPopup` or during initial project setup via `InitRepoPopup`
