...
- `CredentialsPopup` — manage commit author email for an already-registered project (email-only; remote credentials are managed per-remote in `RemotesPopup`)
- `RemotesPopup` — `CardLayout`-based popup for managing git remotes. Card 1 ("List") shows a table of configured remotes with Add/Edit/Remove/Close buttons. Card 2 ("Form") has remote name + URL fields with dynamic HTTPS/SSH credential panels (same pattern as `InitRepoPopup`). Callbacks: `onAddRemote`, `onEditRemote`, `onRemoveRemote`, `onRefresh`. Accessible from the status bar remotes button
...
- Right-click a commit row to show context menu with "Revert Commit" option via `onRevertRequested` callback
...
- `GitManager` — core JGit operations (clone, fetch, pull, push (current branch only by default; `pushAllBranches`/`pushTags`/`forcePush` flags available; push results are checked for `RemoteRefUpdate.Status` — non-fast-forward rejections trigger a force-push confirmation dialog in the Designer), commit (with `amend` flag for replacing the last commit via `CommitCommand.setAmend(true)`), status, branch list/create/checkout/delete with per-branch stash/restore, resource diff content extraction, commit history log with ref decorations for current branch, commit file list, commit file diff, discard changes, revert commit (creates a new commit undoing target commit's changes; aborts cleanly on conflicts via hard reset), remote management (list/add/remove/setUrl via JGit `RemoteAddCommand`/`RemoteRemoveCommand`/`RemoteSetUrlCommand`/`RemoteListCommand`), per-remote credential lookup from `GitRemoteCredentialsRecord`). Remote-dependent operations (`pullImpl`, `pushImpl`, `getRepoURLImpl`, `setupLocalRepoImpl`) are guarded by `GitProjectsConfigRecord.hasRemote()` — local-only repos skip remote operations gracefully. `setAuthentication(command, project, user, remoteName)` is the sole authentication method — looks up per-remote credentials from `GitRemoteCredentialsRecord` and throws if not found (no fallback). Push/pull accept a `remoteName` parameter and use it for both the JGit remote target and credential lookup. Auth type (SSH vs HTTPS) is determined from the remote's URL in `.git/config`
...
**Persistence** uses Ignition's SimpleORM with three record types:
- `GitProjectsConfigRecord` — maps Ignition projects to git repos; `hasRemote()` returns `true` if URI is non-null and non-empty (empty string `""` is the sentinel for local-only repos); `isSSHAuthentication()` returns `false` when `!hasRemote()`. URI is kept in sync with the "origin" remote — adding/removing/editing "origin" in `RemotesPopup` updates this field
- `GitReposUsersRecord` — maps Ignition users to commit author identity (email). Also retains username/password/SSH key columns for schema compatibility, but auth is handled exclusively via `GitRemoteCredentialsRecord`
- `GitRemoteCredentialsRecord` — per-(project, user, remote) auth credentials: username, password (EncodedStringField), SSH key. Created when adding a remote via `RemotesPopup` or during initial project setup via `InitRepoPopup`
