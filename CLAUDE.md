...
**Dockable History panel** (`HistoryPanel.java`): commit log walked from *all* local and remote-tracking branches (`git log --all` style — so divergent branches render as distinct graph lanes and fetched commits show with remote ref badges before merge); borderless toolbar Refresh/Push/Fetch/Pull; a leading **Graph column** (vscode-git-graph-style swimlane DAG — `computeGraph()` assigns each commit to a lane from the `parents` dataset column via incremental lane-tracking, `GraphRenderer` paints colored lane lines + a node dot, recomputed over all loaded rows on each `setData`); Refs column rendered as colored badges; double-click → `CommitDetailPopup`; right-click → Checkout/Revert; "Load More" pagination; thread-safe `setData`. The graph needs parent hashes, so `getCommitLog`/`getCommitHistory` emit a space-separated `parents` column.
